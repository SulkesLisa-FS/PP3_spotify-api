...
class SpotifySearch {
  // Search: artist, albums, and tracks using Spotify Web API
  ...
  async searchItems(searchParams, accessToken) {
    try {
      ...
      searchUrl.searchParams.append(
        "type",
        searchParams.type || "artist,album,track"
      );
      //   limit range
      ...
      const response = await axios.get(searchUrl.toString(), {
        // Pass the user token to tell Spotify who is making the request
  ...
  formatSearchResults(spotifyData) {
    return {
      // ARTIST:
      artists:
        spotifyData.artists?.items?.map((artist) => ({
          id: artist.id,
          name: artist.name,
  ...
  getImageUrl(images, targetSize = 300) {
    ...
    const sortedImages = [...images].sort(
      (a, b) => Math.abs(a.width - targetSize) - Math.abs(b.width - targetSize)
    );
